...
import { FullItemType } from "@/types/invoice.type";
import { Category } from "@prisma/client";
import { useQuery, keepPreviousData } from "@tanstack/react-query";
import { format } from "date-fns";

const ROUTE = ANALYTICS_ROUTES.CATEGORY_SALES.URL;
const KEY = ANALYTICS_ROUTES.CATEGORY_SALES.KEY;
const SUB_KEY = ANALYTICS_ROUTES.CATEGORY_SALES.SUBKEY;
...
export type ApiResponse = {
    itemSales: FullItemType[],
    categorySales: { id: string, name: string, icon: string, quantity: number, price: 0 }[],
    highestItemSale: FullItemType,
    highestCategorySale: { id: string, name: string, icon: string, quantity: number, price: 0 },
};
